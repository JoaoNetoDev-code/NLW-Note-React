...
const NoteCard = ({note, onNoteDelete}: INodeCardProps) => {
  return (
    <Dialog.Root>
      <Dialog.Trigger
      ...
      <span
        ...
        <p
          className="text-sm leading-6 text-slate-400">{note.content}</p>
    ...
    <Dialog.Portal>
        ...
        className="text-sm font-medium text-slate-300">
          ...
          <button onClick={() => onNoteDelete(note.id)} type="button" className="w-full bg-slate-800 py-4 text-center text-sm text-slate-300 outline-none font-medium group">
